...
- **合成シナリオで検証済み** — 629テスト合格（28業界の例示シナリオ × 特許4機能の560テスト含む）。シナリオは「構造的に稀少なノードが ground-truth として価値あるノードと一致する」ように構築された合成グラフである。本検証が立証していること・していないことの詳細は [適用範囲](docs/applicability.md) 参照。
...
28業界シナリオはすべて **合成グラフ** であり、テストデータ生成器が
"truth" ノードを低 weight・低次数（構造的に稀少）として、"garbage" ノード
を孤立として意図的に構築している。したがって本テストスイートは
「構造的稀少性が業務上の重要度と一致する場合に、本アルゴリズムが
構造的に稀少なノードを保護する」ことを示しているが、これは
**特定の業界の実データで成立する保証のある条件ではない**。

実世界での適用可能性は「グラフの構造的稀少性が課題重要度と相関するか」
に依存する。この相関は一部の領域で実データ上確認されている一方
（git-archive のマージコミット、non-scale-free グラフ上の path-critical
bottleneck 等）、他の領域では実データ上反証されている（scale-free な
SNS / 引用ネットワークでは高次数 hub が重要、密度ベースの fraud 検出、
一般 semantic retrieval 等）。

28シナリオの率直な分類と実データで判明している適用境界については
[docs/applicability.md](docs/applicability.md) 参照。実運用での適用前
